...
class DatafileStorage(object):
    ...
    def __init__(self, titles_file, prefixes_file,
                       coordinates_file, data_files_schema):
        """Creates a new DatafileStorage object with relevant files at the
        given locations.

        Note that data_files_schema is a file schema to be used as
        data_files_schema % i, where i is an unsigned integer.

        Usual values for the parameters:
           titles_file = "titles.idx"
           prefixes_file = "prefixes.idx"
           coordinates_file = "coordinates.idx"
           data_files_schema = "wikipedia_%02d.dat"
        """
        ...
        try:
            ...
            self.titles_file_size = 0
        self.prefixes_file = prefixes_file
        try:
            with open(self.prefixes_file, 'rb') as f:
    ...
    def get_titles_with_prefix(self, prefix):
        ...
        characters = evopediautils.characters
        chars_search = dict([(c, i) for (i, c) in enumerate(characters)])

        filepos = 0
        prefix_used = ''
        f_prefixes = self.prefixes
        if 1:
            while True:
                table = f_prefixes[filepos:filepos + 4 + len(characters) * 4]
                filepos += 4 + len(characters) * 4
                if prefix == '':
                    (p,) = struct.unpack("<i", table[:4])
                else:
                    i = chars_search[prefix[0]]
                    data = table[(i + 1) * 4:(i + 2) * 4]
                    (p,) = struct.unpack("<i", data)

                if p > 0:
                    # cannot occur for prefix == ''
                    #f_prefixes.seek(p - 1, os.SEEK_CUR)
                    filepos += p - 1
                    prefix_used += prefix[0]
                    prefix = prefix[1:]
                elif p == 0:
                    return
                else:
        ...
        fullprefix = prefix_used + prefix
        for (title, articlepos) in self.titlestream_at_offset(title_offset):
            title_n = evopediautils.normalize(title)
            if not title_n.startswith(prefix_used):
                return
            if title_n.startswith(fullprefix):
    ...
    def generate_index(self, articles, redirects, max_articles_per_prefix):
        ...
        print "Writing prefix index..."
        self.write_prefix_index()
    ...
    def write_prefix_index(self):
        self.normalized_titles = [title[1] for title in self.titles]
        with open(self.prefixes_file, 'wb') as f_prefixes:
            idx_table = self.get_prefix_index_tables('', 0, len(self.titles))
            f_prefixes.write(idx_table)

#Data structure:
#pointer table:
#    one 4-byte signed integer x:
#        zero: no titles with this prefix
#        negative: first title with this prefix is at byte
#                  position -x - 1 after this table
#        positive: no meaning
#    one 4-byte signed integer x for every character in the list:
#        zero: no titles with this prefix
#        negative: first title with this prefix is at byte
#                  position -x - 1 after this table
#        positive: another pointer table is at byte position x - 1
#                  after current table

    def get_prefix_index_tables(self, prefix, lo, hi):
        import bisect

        characters = evopediautils.characters
        positions = [None] * len(characters)

        for (i, c) in enumerate(characters):
            prefix_new = prefix + c
            pos = bisect.bisect_left(self.normalized_titles,
                                        prefix_new, lo, hi)
            if pos < hi and self.normalized_titles[pos].startswith(prefix_new):
                positions[i] = pos

        pointer_table = struct.pack('<i', - (1 + self.titles[lo][2]))
        following_data = ''
        for (i, pos) in enumerate(positions):
            if pos is None:
                #print " %s: None found" % characters[i]
                pointer_table += struct.pack('<i', 0)
            else:
                next_pos = min([p for p in positions[i + 1:]
                                        if p is not None] + [hi])
                #print " %s: next pos is %d" % (characters[i], next_pos)
                if next_pos - pos <= self.max_articles_per_prefix:
                    pointer_table += struct.pack('<i',
                                        - (1 + self.titles[pos][2]))
                else:
                    c = characters[i]
                    pointer_table += struct.pack('<i', 1 + len(following_data))
                    following_data += self.get_prefix_index_tables(
                                                prefix + c, pos, next_pos)
        return pointer_table + following_data
    ...
    def convert_image(self, image_dir):
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        ...
        backend = DatafileStorage('titles.idx', 'prefixes.idx',
                                  'coordinates.idx', 'wikipedia_%02d.dat')

        if sys.argv[1] == '--convert':
            backend.convert_image(sys.argv[2])
